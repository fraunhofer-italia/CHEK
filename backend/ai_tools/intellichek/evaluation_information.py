"""
This module defines a specialized AI assistant Chatbot named Intellichek, designed to guide users 
through building permit processes and evaluate information based on a maturity model.
Utilizing the langchain library, Intellichek offers expertise in this domain, providing smooth 
and informed assistance. 
The module includes functions to facilitate AI chat interactions with users, specifying responses 
in the user's chosen language, and assesses user-provided tasks, events, and gateways according 
to predefined maturity models.

The primary function, `evaluate_information`, performs asynchronous evaluations of user inputs, 
rating them based on Key Maturity Areas (KMAs) and providing justifications for each rating. 
...
path_to_maturity_info = "./ai_tools/chek_database/maturity_information.txt"
with open(path_to_maturity_info, 'r') as file:
    maturity_info_content = file.read()
...
async def evaluate_information(
    ...
    with get_openai_callback() as cb:
        response = await chain.ainvoke({ "input": inputs, "language": language, "context": maturity_info_content})
